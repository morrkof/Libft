...
------|-------|-------|
**ft_memset** | _void \*ft_memset(void *s, int c, size_t n)_ 
Fills the first `n` bytes of the memory area pointed to by `s` with the constant byte `c`. | A pointer to the memory area `s`.
